...
create table public.leads (
  ...
  converted_to_project  uuid,       -- FK added after projects table is created
  notes                 text,
...
create table public.projects (
  ...
  payment_structure     text,       -- milestone | monthly | upfront
  currency              text        default 'USD',

  -- Status
  phase                 text        default 'discovery',  -- discovery | design | build | review | launch | live
...
-- Add FK from leads back to projects (circular reference — must come after projects)
...
create table public.activities (
  id              uuid        primary key default gen_random_uuid(),
  created_at      timestamptz default now(),
  type            text        not null,   -- call | email | meeting | note | whatsapp
...
-- ============================================================
-- 8. Auth Trigger — Whitelist Enforcement
-- ============================================================
...
-- leads: allow anonymous read/write for dev
create policy "Allow all access for dev" on public.leads
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.contacts
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.projects
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.invoices
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.activities
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.pipeline_stages
  for all using (true) with check (true);

create policy "Allow all access for dev" on public.allowed_users
  for all using (true) with check (true);

-- ============================================================
-- 10. Auto-generated project reference (e.g. CRM-2026-001)
-- ============================================================
